...
--    from the Brands table (Hint: you can use LIMIT and ORDER BY).
 SELECT * FROM Brands
   JOIN Models ON Models.name = brand_name
   ORDER BY year
   LIMIT 3;
...
-- Question: are we supposed to just list each brand once? that's why I did GROUP BY.
...
-- Inner joins only get rows where both tables have values that match in the field you are using to join the tables on.
-- Left joins get all the rows for the first table you select, whether or not there is a matching field in the second table.
